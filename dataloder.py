...
class MiniImagenet(Dataset):
    ...
    def create_batch(self, batchsz):
        ...
        for b in range(batchsz):  # for each batch
            ...
            support_x = []
            query_x = []
            data[str(b)]={'support':[],'query':[]}
            task.append(selected_cls)
    ...
    def read_batch1(self, batchsz,task,num):
        ...
        for b in range(batchsz):  # for each batch
            # 1.select n_way classes randomly
            selected_cls =task.iloc[b]
            
            
            for task_num in range(num):#每个任务类型选num次
                support_x = []
                query_x = []
                data[str(b)+'-'+str(task_num)]={'support':[],'query':[]}
                for cls in selected_cls:
                ...
                data[str(b)+'-'+str(task_num)]['support'].append(support_x) 
                data[str(b)+'-'+str(task_num)]['query'].append(query_x)
            
        print(len(self.support_x_batch))
        self.batchsz=self.batchsz*num
        file = json.dumps(data)
    ...
    def __getitem__(self, index):
        ...
        for idx, l in enumerate(unique):
            #print(len(support_y_relative),len(support_y),self.setsz)
